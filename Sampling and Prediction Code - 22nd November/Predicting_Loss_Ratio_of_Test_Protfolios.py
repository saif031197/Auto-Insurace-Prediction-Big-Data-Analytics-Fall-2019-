...
full_data=full_data.sample(frac=1, random_state=9).reset_index(drop=True)
...
# L1_cv
l1_cv = LassoCV(cv=40, max_iter=10000)
...
l1_count = 0
boost_count = 0
for i in range(len(res_gboost)):
    if res_l1[i] < 0:
        print("Boostion_pred =", res_gboost[i]+res_lgb[i])
        #Sum.append((res_gboost[i]+res_lgb[i])/2)
        Sum.append(res_gboost[i])
    else:  
        print("L1_pred = ",res_l2[i]+res_l1[i])
        ...
        #Sum.append((res_gboost[i]+res_lgb[i] + res_l2[i] + res_l1[i])/4)
        #Sum.append((res_gboost[i] + res_l2[i] + res_l1[i])/3)
